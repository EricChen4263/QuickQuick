...
QuickQuick 是常驻后台的剪贴板/翻译效率工具：开机自启、平时挂在后台，通过全局热键（`Cmd+Shift+C` / `Cmd+Shift+T`）或系统托盘图标唤起窗口。但当前它在 macOS Dock 里**常驻一个应用图标**，与「后台驻留工具」的定位不符——用户反馈其它剪贴板工具（Maccy、Paste 等）都不占 Dock 位。
