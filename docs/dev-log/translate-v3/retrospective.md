...
## 晋升落地确认
- **TV3-RETRO-1 [晋升机制] 部分落地**：已落项目本地 `docs/dev-log/hints.md`（coder 交付前必须实跑全量测试，cargo check 不够），对本项目 TV4 即时避坑；`promoted: [TV3-RETRO-1]` 已回填。**全局晋升到 coder 契约/code-standards 被权限闸门拦截（self-modification），列为待用户批准项**——TV4 启动前置门禁：项目本地已落地、本项目内不阻塞开版；全局复用待用户批准后补落。
- 往期未落地 [晋升机制]：TV1-RETRO-1（已落地）、TV2-RETRO-1（项目本地已落、全局待批准）——无新增遗留。
