...
## 晋升落地确认
- **TV2-RETRO-1 [晋升机制] 部分落地**：已落项目本地 `docs/dev-log/hints.md` Hint 段（「证否密钥不泄露须用非空 sentinel 脏值」+ 附带「复杂签名独立复算锚定」），对本项目 TV3/TV4 即时避坑生效；`promoted: [TV2-RETRO-1]` 已回填。**全局晋升到 code-standards §8 被权限闸门拦截（self-modification），列为待用户批准项**——TV3 启动前置门禁：项目本地已落地、本项目内不阻塞开版；全局复用待用户批准后补落。
