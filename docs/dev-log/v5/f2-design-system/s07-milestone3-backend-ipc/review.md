...
逐条核实首轮必改项在 commit `60a9645`（fix(backend): 排除名单运行时即时生效 + themeStore 死代码清理，里程碑3 review）中的落实情况。

| 初审问题 | 修复（已核实位置） | 结论 |
|---|---|---|
| M1（Critical）：`ExcludeList` 运行时永不生效，`start_clipboard_poll` 启动时创建空名单后不更新，`set_exclude_list` IPC 仅写文件，对运行中轮询无效 | `src-tauri/src/lib.rs`：`CaptureState` 新增 `pub exclude_list: Arc<RwLock<privacy::ExcludeList>>`（第 54 行）；`init_capture_state` 从 settings 初始化该字段（第 355-363 行）；`start_clipboard_poll` 接受 `Arc<RwLock<privacy::ExcludeList>>` 参数（第 188 行），每轮循环 `exclude_list.read()` 获取当前名单（第 218 行），构造 `CapturePolicy` 时实时使用；`src-tauri/src/ipc/settings.rs`：`set_exclude_list_impl` 写文件成功后，通过传入的 `runtime_list: Option<&RwLock<...>>` 同步替换运行时（第 141-160 行），`set_exclude_list` 命令传入 `Some(&state.exclude_list)` 触发即时更新（第 276 行）；测试 `set_exclude_list_updates_runtime_immediately` 验证了文件写入和运行时更新的原子一致性 | 已修复 |
| Important：`themeStore.ts` `init()` 第 91 行 `typeof window !== "undefined" &&` 冗余（早返回后永不到达，死代码） | `src/theme/themeStore.ts:91` 改为 `if (window.matchMedia) {`，去除冗余 `typeof window !== "undefined" &&` 检查，依赖第 87 行的早返回即可 | 已修复 |
