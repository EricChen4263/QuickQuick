...
level: 大功能
parent: V5
children: [f4-s01-tray-icon, f4-s02-clip-translate-jump, f4-s03-image-threshold, f4-s04-auto-paste, f4-s05-auto-update, f4-s07-close-to-tray, f4-s08-clip-autorefresh, f4-s09-unmount-race, f4-s10-event-const, f4-s11-trans-history-autorefresh, f4-s12-trans-degenerate-pair, f4-s13-settings-cleanup, f4-s14-hotkey-capture, f4-s15-hotkey-live-register]
...
| s13 设置页占位清理 | 设置页端到端审计（6 分区逐项追踪）收口：移除热键页「回车粘贴」纯本地占位开关（无 IPC、刷新即丢、不影响行为）；关于页版本号由硬编码 v1.0.0 改读 getVersion() 真实值（v0.0.1）。审计结论：9 项可用、1 项外部阻塞（自动更新 endpoint）、本 s13 清 1 占位+1 瑕疵 | tester 命中+2 变异（加回占位/改回硬编码）全红再复绿、SettingToggle 本体未误伤；reviewer 通过无≥80问题；前端 366 全绿 | d6c9ae8 |
| s14 热键按键捕获 | 热键改键从「手打加速键串」改为「按键捕获」：点「修改」进入录制→按组合键即捕获显示→保存。新增纯函数 keyEventToAccelerator（e.code 归一、修饰键定序 CmdOrCtrl→Ctrl→Alt→Shift、≥1修饰键+主键否则 null）；HotkeyRow 录制状态机（Esc 取消、preventDefault、保存复用 validateRebind+setHotkey）；后端不改 | tester 命中+3 变异（修饰守卫/字母映射/捕获→保存接线）全红再复绿、纯修饰/Esc/保存禁用/冲突校验全覆盖；reviewer 通过无≥80问题；前端 400 全绿 | 950cfdc |
| s15 改热键运行时即时生效 | 既有 backend 缺陷：set_hotkey 只持久化+冲突检测，从不运行时注册新键/注销旧键（回调只启动期绑一次）→ 改键后新键无反应、旧键仍触发、要重启才生效。lib.rs 抽 register_action_shortcut（启动期与改键复用、绑 popover 回调）；set_hotkey 命令读旧键→持久化→注销旧→注册新 | tester 319 全绿连跑3次无flaky+build exit0+2 label 测试命中+2变异红复绿、次序稳健性评估可接受；reviewer 通过无≥80问题；运行时生效需 GUI 实测 | ad8cc62 |
...
- **s14 热键按键捕获**：设置→热键→点某条的「修改」→直接按下组合键（如 ⌘⇧Y）应即时显示捕获结果→点「保存」生效；按 Esc 或「取消」应还原；只按修饰键不应能保存。（纯前端，重新 build 后可测）
- **s15 改热键运行时即时生效**：改键并保存后**不重启** app，直接按**新键**应弹出对应 popover、**旧键**应失效（不再触发）。（改了 Rust，需重新 build）
