...
发版前本地 release 出包实测，对 B 视觉做了多轮校准、并新增 E：

- **B 标题栏红绿灯**：`trafficLightPosition` config 在 `visible:false` 窗口失效，改用底层 NSWindow API（`ns_window()` + `standardWindowButton` + `setFrameOrigin`）在 setup 内显式重定位三颗按钮（`src-tauri/src/lib.rs` `traffic_light_logical_position()` + `reposition_traffic_lights()`，objc2 feature 见 Cargo.toml）。坐标经实测迭代定为 `(18, 12)`（几何居中 38px 栏）；`.qq-titlebar` padding-left 76→96px 留空、font-size 13→15px 调大。新增 `tests/traffic_light.rs` 2 条纯函数测试。用户实测确认：居中、留空、字号均 OK。
- **E 翻译按钮置灰**：翻译页按钮 React 层早有 `disabled`+「翻译中…」，但 `.btn` 无 `:disabled` 样式致禁用态外观无变化。`src/theme/components.css` 加 `.btn:disabled { opacity:0.5; cursor:not-allowed }`、hover/active 改 `:not(:disabled)`，禁用态明显置灰且无悬停反馈。
- **更新失败（非代码）**：实测「检查更新」报错，根因=仓库私有致 release 资产匿名 404；已将仓库设为 public，endpoint 与下载链接匿名 200，updater 恢复正常。

Phase 6（B+E）：code-reviewer APPROVE（NSWindow 重定位 unsafe 安全性/坐标换算/cfg 对称逐项过）；tester 放行（traffic_light 2 测试命中、变异全红、make verify cargo 393 / vitest 453 全绿、工作树净）。
