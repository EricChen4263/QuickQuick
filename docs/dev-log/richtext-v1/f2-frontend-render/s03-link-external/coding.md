...
## 打回修复 #1（RT1-M01 真机暴露的运行时缺陷）
- 现象：点链接"没反应"。根因：`opener:allow-open-url` 只放行 open_url 命令、**无 URL scope**，运行时所有 URL 被 ACL 拒、openUrl 抛错被 `void` 静默吞。
- 修复：① capability 补 `opener:allow-default-urls`（提供 http/https/mailto/tel scope，最小授权不引入 reveal/open-path）；② `handleRichLinkClick` 改 `.catch(console.error)` 不再静默吞错，新增测试 `rich_link_click_logs_error_when_open_rejects`。
- 验证：cargo test 531/0 failed（boot_smoke 反序列化守卫过）、pnpm 483 passed、tsc/clippy 干净；tester 删 .catch 变异 RED 证判别力。ACL scope 真机放行最终归 RT1-M01。
