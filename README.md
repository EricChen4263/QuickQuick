...
- **默认热键**：剪贴板 `Cmd/Ctrl+Shift+C` · 翻译 `Cmd/Ctrl+Shift+T` · 主界面 `Cmd/Ctrl+Shift+M`（可在设置里改键）。
...
- **Default hotkeys**: clipboard `Cmd/Ctrl+Shift+C` · translate `Cmd/Ctrl+Shift+T` · main window `Cmd/Ctrl+Shift+M` (rebindable in Settings).
