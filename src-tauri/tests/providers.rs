...
/// A11-1: MyMemory 撞额度 → 返回 NeedEmail 提示（引导填邮箱提升配额）
#[test]
fn quota_explicit_no_silent_switch_mymemory_quota_returns_need_email() {
    // Arrange
    let err = TranslateError::Quota("MyMemory 配额耗尽".to_string());

    // Act
    let prompt = on_quota_or_failure("mymemory", &err);

    // Assert: 必须返回显式提示，类型为 NeedEmail
    let p = prompt.expect("撞额度必须返回显式用户提示，不得返回 None");
    assert_eq!(
        p.kind,
        UserPromptKind::NeedEmail,
        "MyMemory 配额耗尽应引导填邮箱，实际: {:?}",
        p.kind
    );
